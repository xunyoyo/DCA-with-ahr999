# trade_bot.py (Final Safety-Patched Version)
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Daily Investment Bot for OKX with Persistent Logging, Charting & Portfolio Summary
------------------------------------------------------------------------------------
This is a safety-patched version that FIXES two critical bugs:
1.  [CRITICAL FIX] Removed the dangerous "Pulse" logic that caused over-investment.
    The bot will now NEVER invest more than the calculated multiplier amount.
2.  [ROBUSTNESS FIX] Added comprehensive checks for empty log files to prevent crashes
    during the first run or if the log is cleared.
"""
...
BETA = 0.8
DAILY_CAP_X = 4.0
# [FIXED] PULSE_THRESHOLD is now for reference only, the additive logic is REMOVED.
...
def generate_roi_chart():
    # [ROBUSTNESS FIX] Check if file exists AND is not empty before proceeding.
    if not os.path.exists(LOG_FILE) or os.path.getsize(LOG_FILE) == 0:
        print("Log file not found or is empty, skipping chart generation.")
        return
    try:
        log_df = pd.read_csv(LOG_FILE)
        if len(log_df) < 2:
            print("Not enough data to generate a chart (need at least 2 entries).")
            return
        ...
        print(f"Could not generate chart due to an error: {e}")
...
def get_today_investment_amount(historical_df: pd.DataFrame, baseline: float) -> dict:
    ...
    if np.isfinite(ahr999_today):
        if ahr999_today > PAUSE_THRESHOLD:
            ...
            # [CRITICAL FIX] The dangerous additive pulse logic has been REMOVED.
            # The high multiplier in the pulse zone is aggressive enough.
            # No more "buy_usd_ahr += baseline"
...
def main():
    ...
    try:
        ...
        if investment_amount > 1:
            ...
            execution_log = (f"### 📈 Trade Execution\n- **Status:** `SKIPPED`\n- **Reason:** Calculated amount is below the minimum trade size of $1.")
